...
app1(){
  ...
  for(i=0;i<5;i++){
    ...
    sum=class_A[i].ch_score+class_A[i].en_score+class_A[i].math_score+class_A[i].nr_score+class_A[i].soc_score;
    cout<<setw(6)<<sum<<endl;
  }
  cout<<endl<<"英文&數學成績皆不及格的學生名單有："<<endl;
  cout<<setw(8)<<"姓名"<<setw(7)<<"學號"<<setw(6)<<"英文";
  cout<<setw(6)<<"數學"<<endl;
  for(i=0;i<5;i++){
    if(class_A[i].en_score<60 && class_A[i].math_score<60){
      cout<<setw(8)<<class_A[i].chinese_name;
      cout<<setw(7)<<class_A[i].id_number;
      cout<<setw(6)<<class_A[i].en_score;
      cout<<setw(6)<<class_A[i].math_score<<endl;
    }
  }
  system("Pause");
}
